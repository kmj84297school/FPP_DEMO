...
def cached_player_result(player_id: str):
    start = time.time()
    outroot = str(Path("viz_out") / f"viz_{player_id}")   # ★ 추가: 항상 viz_out 안으로
    result = analyze_player(player_id, outroot=outroot)   # ★ outroot 전달
...
async def analyze(request: Request, player_id: str = Form(...)):
    ...
    # app.py (/analyze 라우트의 except 블록)
    except Exception:
        outroot = str(Path("viz_out") / f"viz_{player_id}")   # ★ 동일
        result = analyze_player(player_id, outroot=outroot)   # ★ 동일
        result["_cached"] = False
        result["_elapsed"] = round(time.time() - t0, 2)
