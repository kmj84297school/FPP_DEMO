"""2023 스냅샷 중 예측모델 적용 대상(eligible) 선수에 대해:
- 잔존확률 (models/xgb_survival_v2.json)
- 예측 미래능력 mu (models/xgb_ability_v2.json)
- 신뢰구간 (부트스트랩 앙상블 sigma_model + 저장된 sigma_residual=7.49 결합)
을 계산한다. 특징행렬(features_2023_all.csv)은 04/05 단계에서도 재사용된다.
"""
import sys, pathlib
...
if __name__ == "__main__":
    ...
    feat_all.to_csv(CACHE / "features_2023_all.csv")
    print("features_2023_all:", feat_all.shape)
    ...
    out.to_csv(CACHE / "predictions_2023.csv", index=False)
    print("저장 완료:", out.shape, "->", CACHE / "predictions_2023.csv")
