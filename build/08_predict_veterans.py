"""대상 시즌 스냅샷의 24세+ '베테랑' 선수에 대해 전성기 유지 예측
...
from features import reindex_for_model, MODEL_FEATURES
from ensemble import fit_ensemble, predict_ensemble
from config import (DATA, ABILITY_CSV, ELIGIBILITY_META, FEATURES_CURRENT_CSV,
                    PRED_PEAK_CSV, VETERAN_META, TARGET_YEAR)
...
if __name__ == "__main__":
    ...
    out.to_csv(PRED_PEAK_CSV, index=False)
    print("저장 완료:", out.shape, "->", PRED_PEAK_CSV)
    ...
    with open(VETERAN_META, "w", encoding="utf-8") as f:
