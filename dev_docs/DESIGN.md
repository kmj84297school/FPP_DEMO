...
- **궤도 게이지**(`orbitGaugeHtml`): 0~100 궤도 위에 현재(초록 점) → **예측 중심값(속 빈 원)** → 상한(금색 별) 세 점.
  상한만 보고 판단하는 것을 막기 위한 장치다. 카드에서는 게이지 아래에 "중심 83.7"만 덧붙이고(현재·상한은 이미 큰 숫자),
  상세·비교 헤더에서는 "현재 · 중심 · 상한" 세 라벨을 모두 쓴다.
...
- **상한과 중심값을 항상 함께 보여준다.** 상한은 상위 10% 시나리오, 중심값은 가장 가능성이 높은 값 — 상세 페이지의
  예측 패널은 상한 배지(채움) 바로 아래에 중심값 배지(아웃라인)와 그 차이를 설명하는 한 줄을 둔다. 목록의 상한 정렬에서도
  배지 아래에 "중심 81.2"를 병기한다.
