...
# (파일 상단의 유틸 근처에 추가)
def extract_club_from_profile_html(html: str) -> str | None:
    clean = strip_html_comments(html)
    # 1) "Club:" 라벨 뒤 앵커 텍스트
    m = re.search(r'(Club:)\s*</(?:strong|span)>\s*<a[^>]*>([^<]+)</a>', clean, flags=re.I)
    if m:
        return re.sub(r'\s+', ' ', m.group(2)).strip()
    # 2) 라벨 뒤 일반 텍스트 백업
    m2 = re.search(r'(Club:)\s*</(?:strong|span)>\s*([^<]+)<', clean, flags=re.I)
    if m2:
        return re.sub(r'\s+', ' ', m2.group(2)).strip()
    return None

def extract_face_image_url_from_profile_html(html: str) -> str | None:
    clean = strip_html_comments(html)
    # FBref 헤더의 og:image 메타 사용 (가장 안정적)
    m = re.search(r'<meta[^>]+property=["\']og:image["\'][^>]+content=["\']([^"\']+)["\']', clean, flags=re.I)
    if m:
        return m.group(1).strip()
    # 백업: 프로필 카드 내 img(있다면)
    m2 = re.search(r'<img[^>]+src=["\']([^"\']+)["\'][^>]*class=["\'].*?(player-headshot|face).*?["\']', clean, flags=re.I)
    if m2:
...
def try_online_fetch(driver, player_id: str):
    ...
    club = extract_club_from_profile_html(profile_html)
    face_img = extract_face_image_url_from_profile_html(profile_html)
        ...
        for cohort in COHORTS:
            for slug in slug_cands[:5]:
                ...
                if wb and ("Statistic" in wb or "Percentile" in wb):
                    return wb, chosen_url or guess, player_name, dob, pos_text, role, club, face_img
        raise RuntimeError("Slug/검색/아카이브 모두 실패 — 스카우팅 리포트에 접근 불가")
    return html, chosen_url, player_name, dob, pos_text, role, club, face_img

def run_online(player_id: str, headless=True):
    driver = start_driver(headless=headless)
    try:
        # ✅ 8개 언팩 (HTML, 원본URL, 이름, 생년, 포지션텍스트, 역할, 클럽, 얼굴이미지)
        html, src_url, player_name, dob, pos_text, role, club, face_img = try_online_fetch(driver, player_id)
        ...
        if club:
            meta["club"] = club
            # 템플릿의 League/Team 표시에 맞추어 team 키도 채워줍니다.
            meta.setdefault("team", club)
        if face_img:
            meta["face_image_url"] = face_img
...
def analyze_player(player_id: str,
    ...
    outroot = outroot or f"viz_out/viz_{player_id}"
...
"""
    ...
    try:
        ...
        pass
        # --- [PATCH] viz_info -> 대시보드용 웹 경로(paths) 생성 ---
    viz_paths = {}
    try:
        vdir = (viz_info or {}).get("dir")
        vfiles = (viz_info or {}).get("files", [])
        if vdir and vfiles:
            vname = Path(vdir).name  # viz_ae4fc6a4 같은 폴더명
            for fname in vfiles:
                if fname.lower().endswith(".png"):
                    key = Path(fname).stem  # strengths / weaknesses / groups / radar
                    viz_paths[key] = f"/viz/{vname}/{fname}"
    except Exception:
        viz_paths = {}
    ...
    return {
        ...
        "viz": {"paths": viz_paths, **(viz_info or {})},
